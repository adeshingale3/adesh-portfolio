...
const Projects = () => {
    ...
    return (
        <div className='mt-3 ml-0 flex flex-col gap-4 '>
            {projects.map((prj, key) => {
                return (
                    <div key={key} className='w-full rounded-lg p-4 border-1 border-white/10 rounded-md' style={{background: 'transparent', borderTop: '1px solid rgba(255,255,255,0.04)'}}>
                        <div className='flex flex-row justify-between items-start'>
                            <div>
                                <h2 className='font-semibold text-[var(--foreground)]'>{prj.name}</h2>
                                <h4 className='italic text-[14px] mt-1 text-[var(--muted)]'>{prj.description}</h4>
                            </div>
                            <div className='flex flex-row gap-2'>
                                <a href={prj.github} className='h-8 w-8 flex justify-center items-center rounded-sm' style={{color: 'var(--foreground)'}} aria-label="github">
                                ...
                                <a href={prj.live} className='h-8 w-8 flex justify-center items-center rounded-sm' style={{color: 'var(--foreground)'}} aria-label="live">
                        ...
                        <div className=' mt-3 w-full h-auto flex flex-row flex-wrap gap-2'>
                            {prj.techstack.map((tech, key) => {
                            return (
                                <div key={key} className='min-w-auto h-6 flex items-center bg-white/6 text-[var(--muted)] p-1 rounded-sm text-[12px] px-2'>{tech}</div>
                            )
                        })}
                        </div>
